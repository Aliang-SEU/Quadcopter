...
S_INT16_XYZ	GYRO_OFFSET={0,0,0},ACC_OFFSET={56,86,0},MAG_OFFSET={140,159,-260};		//零漂
...
void MPU9250_ReadValue(void)
{
	...
	for(i=0;i<22;i++)//一共读取22字节的数据
	{
		MPU9250_buf[i]=SPI1_ReadWriteByte(0xff); //循环读取 写0xff是因为slave不识别
	}	
	...
	MPU9250_MAG_LAST.X = ((int16_t)MPU9250_buf[16]<<8) | MPU9250_buf[15];
	MPU9250_MAG_LAST.Y = ((int16_t)MPU9250_buf[18]<<8) | MPU9250_buf[17];
	MPU9250_MAG_LAST.Z = ((int16_t)MPU9250_buf[20]<<8) | MPU9250_buf[19];
...
uint8_t MPU9250_Init(void)
{
	...
	if(MPU9250_Read_Reg(WHO_AM_I)==0x71)	//正确读取到9250的地址
	{
...
void MPU9250_IOAndSPI_Init(void)
{
  ...
  SPI_InitTypeDef  SPI_InitStructure;
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4; //狹PU9250片选信号
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOA, &GPIO_InitStructure);	//初始化指定IO
 	//GPIO_SetBits(GPIOB,GPIO_Pin_12);//上拉  禁止SPI通信
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_64;		//定义波特率预分频的值:波特率预分频值为16（72M/16=6M）
...
void Init_AK8963()
{
	...
	OLED_ShowString(0,0,"Reset AK8963!");
	OLED_Refresh_Gram();
	delay_ms(500);
	...
	if(temp == 0x48)
	{
			OLED_ShowString(0,10,"AK8963 Read ID Success!");
			OLED_Refresh_Gram();
			delay_ms(500);
	}
	else 
	{
			OLED_ShowString(0,10,"AK8963 Read ID Fail!");
			OLED_Refresh_Gram();
			while(1);
	}
	...
	if(temp == 0x16)
	{
			OLED_ShowString(0,20,"AK8963	Init Success!");
			OLED_Refresh_Gram();
			delay_ms(500);
	}
	else 
	{
			OLED_ShowString(0,20,"AK8963 Init Fail!");
			OLED_Refresh_Gram();
			while(1);
	}
	OLED_Clear();
